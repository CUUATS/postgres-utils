...
-- headway peak time 06-08
--DROP MATERIALIZED VIEW gtfs_2015.head_way_peak CASCADE;
CREATE MATERIALIZED VIEW gtfs_2015.head_way_peak AS
...
-- headway non-peak time 3-5
DROP MATERIALIZED VIEW gtfs_2015.head_way_non_peak CASCADE;
CREATE MATERIALIZED VIEW gtfs_2015.head_way_non_peak AS 
SELECT 	DISTINCT ON (st.stop_id, arrival_time, shape_id)
		r.route_id,
		st.stop_id, 
		d.service_id,
...
FROM gtfs_2015.routes AS r
	...
	JOIN gtfs_2015.calendar_dates as d
		ON t.service_id = d.service_id
WHERE (st.arrival_time LIKE '15%'
		OR st.arrival_time LIKE '16%'
	  	OR st.arrival_time LIKE '17%')
	AND stop_sequence = 1
	AND (route_long_name NOT LIKE '%Saturday' AND
	route_long_name NOT LIKE '%Sunday' AND
	route_long_name NOT LIKE '%Weekend' AND
...
CREATE MATERIALIZED VIEW gtfs_2015.common_routes_mat AS 
